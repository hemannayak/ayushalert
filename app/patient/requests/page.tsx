...
export default function PatientRequests() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center space-y-4">
      ...
      <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Syncing Request Ledger...</p>
    </div>
  );
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto w-full p-6 lg:p-10 space-y-8 flex-1">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-center justify-between gap-6 border-b border-white/5 pb-8">
           <div className="flex items-center gap-6">
              ...
              <div>
                 <h1 className="text-2xl font-black tracking-tighter uppercase">Access Governance</h1>
                 <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.2em] mt-1">Real-time consent surveillance</p>
              </div>
           </div>
           <div className="flex items-center gap-4">
              <div className="px-4 py-2 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-[9px] font-black uppercase tracking-widest text-emerald-400 flex items-center gap-2">
                 <RefreshCcw size={12} className="animate-spin-slow" /> Ledger Sync: Active
              </div>
           </div>
        </motion.div>
           ...
           {requests.length === 0 && !error && (
             <div className="p-20 rounded-[40px] border border-white/5 bg-zinc-900/20 backdrop-blur-xl text-center space-y-4">
                ...
                <div className="space-y-1">
                   <h3 className="text-sm font-black text-white uppercase tracking-widest">Zero Active Requests</h3>
                   <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest">Your identity node is currently isolated from external clinical syncs.</p>
                </div>
             </div>
           )}
        ...
        <footer className="pt-10 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6">
           <p className="text-[9px] font-black text-zinc-800 uppercase tracking-[0.2em] max-w-xl text-center sm:text-left leading-relaxed">
              Institutional consent framework complying with ABDM guidelines · Access is granular and cryptographically verified upon authorization.
           </p>
           <div className="flex items-center gap-6">
              <div className="flex items-center gap-2"><Lock size={12} className="text-zinc-800" /><p className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">E2E Sovereign Security</p></div>
              <div className="flex items-center gap-2"><Database size={12} className="text-zinc-800" /><p className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">Distributed Ledger</p></div>
           </div>
        </footer>
      </div>
    </div>
...
function ConsentRequestCard({ request }: { request: ConsentRequest }) {
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setOtpSent(true); setMsg('Handshake Code transmitted to registered endpoint.');
    } catch (err: any) { setMsg(err.message); }
    finally { setLoading(false); }
  };

  const handleAction = async (action: 'approve' | 'reject') => {
    if (action === 'approve' && !otp.trim()) { setMsg('Handshake Code required.'); return; }
    setLoading(true); setMsg('');
    try {
      ...
      setMsg(action === 'approve' ? '✅ Synchronization established' : '❌ Request terminated');
    } catch (err: any) { setMsg(err.message); }
    finally { setLoading(false); }
  };
    ...
    <div className={`rounded-[32px] border border-white/5 bg-zinc-900/40 backdrop-blur-xl p-8 space-y-6 transition-all shadow-2xl relative overflow-hidden group`}>
       ...
       {status === 'pending' && (
         <div className="pt-6 border-t border-white/5 space-y-6">
            <div className="p-4 rounded-xl bg-zinc-950/40 border border-white/5 text-[10px] font-bold text-zinc-500 leading-relaxed">
               Verification node <span className="text-indigo-400">{request.hospital_id}</span> is requesting a sovereign handshake to synchronize with your longitudinal health record.
            </div>
            {!otpSent ? (
               <button onClick={sendOtp} disabled={loading} className="w-full py-4 rounded-2xl bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition shadow-xl shadow-indigo-500/20 active:scale-95 disabled:opacity-50">Transmit Verification Code</button>
            ) : (
               <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <button onClick={()=>handleAction('approve')} disabled={loading} className="py-4 rounded-2xl bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-zinc-200 transition shadow-xl shadow-white/5 active:scale-95 disabled:opacity-50">Approve Access</button>
                     <button onClick={()=>handleAction('reject')} disabled={loading} className="py-4 rounded-2xl bg-zinc-800 border border-white/5 text-[10px] font-black uppercase tracking-widest text-rose-500 hover:bg-zinc-700 transition active:scale-95 disabled:opacity-50 uppercase">Terminate</button>
                  </div>
               </div>
            )}
         </div>
